...
--
-- Все идентификаторы переведены на BIGINT / BIGSERIAL + int64 в Go (для устранения int32/int64 кастов).
-- ============================================================================
...
CREATE TABLE kanban_project_user (
    ...
    folder_id         BIGINT REFERENCES kanban_project_user_folder(id) ON DELETE SET NULL,
    position          DOUBLE PRECISION NOT NULL DEFAULT 0,
...
CREATE TABLE kanban_card_label (
    kanban_card_id  BIGINT NOT NULL REFERENCES kanban_card(id) ON DELETE CASCADE,
    kanban_label_id BIGINT NOT NULL REFERENCES kanban_label(id) ON DELETE CASCADE,
